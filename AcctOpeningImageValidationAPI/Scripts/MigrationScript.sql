...
INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210719135723_OcrUsageIdMigration', N'5.0.3');
GO
...
DROP INDEX [IX_FacialValidations_OCRUsageId] ON [FacialValidations];
GO
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[FacialValidations]') AND [c].[name] = N'OCRUsageId');
IF @var1 IS NOT NULL EXEC(N'ALTER TABLE [FacialValidations] DROP CONSTRAINT [' + @var1 + '];');
ALTER TABLE [FacialValidations] ALTER COLUMN [OCRUsageId] int NULL;
GO

CREATE UNIQUE INDEX [IX_FacialValidations_OCRUsageId] ON [FacialValidations] ([OCRUsageId]) WHERE [OCRUsageId] IS NOT NULL;
GO

ALTER TABLE [FacialValidations] ADD CONSTRAINT [FK_FacialValidations_OCRUsages_OCRUsageId] FOREIGN KEY ([OCRUsageId]) REFERENCES [OCRUsages] ([Id]) ON DELETE NO ACTION;
GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20210719151726_OcrUsageIdRemoved', N'5.0.3');
GO
